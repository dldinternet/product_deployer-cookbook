		...
		module Mixin
			...
			module Inventory
				...
				def deployer_getInventory(args)
					...
					# Second get the repo specific data bag
            ...
            begin
              ...
              if matches
				...
				def deployer_getArtifacts(args, inventory)
					#build_drw = "#{args[:product]}-#{args[:version]}-#{args[:branch]}"
					#build_nam = "#{args[:product]}-#{args[:version]}-#{args[:branch]}-build-#{args[:build]}"
					...
					artfct_h.each { |artifact_id|
						...
						artifact[:file]        = "/tmp/#{artifact_fil}"
